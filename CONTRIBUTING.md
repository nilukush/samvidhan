...
0. Automatic nudge: a scheduled GitHub Actions workflow (`.github/workflows/ritual-check.yml`) runs on the first of each month. It fetches the sources, compares against its committed baseline in `data/ritual/detector-state.json`, and opens an issue when the Wikipedia amendment count moves or a tracked bill page changes. The detector only observes; it never writes a verified date. One known limitation, stated in every report it produces: the legislative portal is client-rendered, so the new-edition check there stays a manual look.
1. Constitution text: check legislative.gov.in/constitution-of-india for a new edition PDF (amendments are rare, 1 to 2 a year). If there is a new one, download it into `data/source/`, update that README, rerun the extraction pipeline, spot check 10 articles against the PDF, and update the "text verified" date in the site data and footer.
